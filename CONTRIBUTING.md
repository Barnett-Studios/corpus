...
Thanks for your interest in corpus — the RED-baseline measurement corpus. It is one component of the
...
By contributing you agree that your contributions to the adaptation layer are licensed under
the project's [MIT license](LICENSE) (inbound = outbound); Exercism-derived node content
remains under its upstream licenses — see [ATTRIBUTION.md](ATTRIBUTION.md).
...
```bash
git clone https://github.com/Barnett-Studios/corpus
cd corpus
# no build step — this is a data component
```
...
|---|---|---|
| Attribution | `./verify-attribution.sh --check` | manifest matches the data, no drift |
| RED invariant | each node ships **failing** its acceptance test | enforced |
...
corpus is defined by its [`CONTRACT.md`](CONTRACT.md): the interface, the invariants, and the
...
[security advisory](https://github.com/Barnett-Studios/corpus/security/advisories/new) form so it can be
fixed before disclosure.

---

Questions that don't fit an issue? Open a
[discussion](https://github.com/Barnett-Studios/corpus/discussions). Thanks for contributing.
